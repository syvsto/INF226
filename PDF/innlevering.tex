...
% er en setup lengre oppe for akkurat OpenMRS s¨det trenger vi vel ikke her!
...
required changing the -Xmx setting in the ``/Applications/HP_Fortify/
HP_Fortify_SCA_and_Apps_16.10/Core/private-bin/awb/eclipse/Eclipse.app/
Contents/MacOS/eclipse.ini'' file, which required a bit of dedication to find.
The reason for this is that scanning a project requires more memory than is
allocated to Audit Workbench by default. After that, running Audit Workbench was
...
build. This way IntelliJ could reason about the classes which was not necessary
...
Exploit DB has four relatively new exploits listed for OpenMRS. Of these  
